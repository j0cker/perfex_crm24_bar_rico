...
<div class="panel-body mtop10">
   <div class="row">
      ...
      <div style="display: none;" class="col-md-8 text-right show_quantity_as_wrapper">
         <div class="mtop10">
   ...
   <div class="table-responsive s_table">
      <table class="table estimate-items-table items table-main-estimate-edit has-calculations no-mtop">
         <thead>
            <tr>
               <th></th>
               <th width="20%" align="center"><i class="fa fa-exclamation-circle" aria-hidden="true" data-toggle="tooltip" data-title="<?php echo _l('item_description_new_lines_notice'); ?>"></i> <?php echo _l('estimate_table_item_heading'); ?></th>
               <th width="35%" align="center"><?php echo _l('estimate_table_item_description'); ?></th>
               <?php
               ...
               <th style="display: none;" width="20%" align="center"><?php echo _l('estimate_table_tax_heading'); ?></th>
               <th width="20%" align="center"><?php echo _l('estimate_table_amount_heading'); ?></th>
         ...
         <tbody>
            <tr class="main">
               <td></td>
               <td>
                  <textarea readonly="readonly" name="description" rows="4" class="form-control" placeholder="<?php echo _l('item_description_placeholder'); ?>"></textarea>
               </td>
               <td>
                  <textarea readonly="readonly" name="long_description" rows="4" class="form-control" placeholder="<?php echo _l('item_long_description_placeholder'); ?>"></textarea>
               </td>
               ...
               <td>
                  <input readonly="readonly" type="number" name="rate" class="form-control" placeholder="<?php echo _l('item_rate_placeholder'); ?>">
               </td>
            ...
            <?php if (isset($estimate) || isset($add_items)) {
               ...
               $table_row .= '</td>';
               $table_row .= '<td class="bold description"><textarea readonly="readonly" name="' . $items_indicator . '[' . $i . '][description]" class="form-control" rows="5">' . clear_textarea_breaks($item['description']) . '</textarea></td>';
               $table_row .= '<td><textarea readonly="readonly" name="' . $items_indicator . '[' . $i . '][long_description]" class="form-control" rows="5">' . clear_textarea_breaks($item['long_description']) . '</textarea></td>';
               ...
               $table_row .= '<input style="display: none;" type="text" placeholder="'.$unit_placeholder.'" name="'.$items_indicator.'['.$i.'][unit]" class="form-control input-transparent text-right" value="'.$item['unit'].'">';
               $table_row .= '</td>';
               $table_row .= '<td class="rate"><input readonly="readonly" type="number" data-toggle="tooltip" title="' . _l('numbers_not_formatted_while_editing') . '" onblur="calculate_total();" onchange="calculate_total();" name="' . $items_indicator . '[' . $i . '][rate]" value="' . $item['rate'] . '" class="form-control"></td>';
               $table_row .= '<td style="display: none;" class="taxrate">' . $this->misc_model->get_taxes_dropdown_template('' . $items_indicator . '[' . $i . '][taxname][]', $estimate_item_taxes, (isset($is_proposal) ? 'proposal' : 'estimate'), $item['id'], true, $manual) . '</td>';
               $table_row .= '<td class="amount" align="center">' . $amount . '</td>';
   ...
   <div class="col-md-8 col-md-offset-4">
      <table class="table text-right">
         <tbody>
            <tr style="display: none;" id="subtotal">
               <td><span class="bold"><?php echo _l('estimate_subtotal'); ?> :</span>
               </td>
               <td class="subtotal">
               </td>
            </tr>
            <tr style="display: none;" id="discount_area">
               <td>
                  <div class="row">
                     <div class="col-md-7">
