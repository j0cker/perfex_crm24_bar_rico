...
class Reports extends AdminController
{
        ...
        */
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                ...
                'YEAR(date) as year',
                'date',
                '' . db_prefix() . 'estimates.datecreated as datecreated',
                'subtotal',
                'total',
                'total_tax',
                'discount_total',
    ...
    {
        /*
        /admin/reports/sales -> artículos

        informes -> ventas -> artículos
        */
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($v && strpos($v->version, '5.7') !== false) {
                $aColumns = [
                        ...
                        '' . db_prefix() . 'itemable.rel_type as tipo',
                    ];
            } else {
                $aColumns = [
            ...
            $join         = ['LEFT JOIN ' . db_prefix() . 'proposals ON ' . db_prefix() . 'proposals.id = ' . db_prefix() . 'itemable.rel_id AND ' . db_prefix() . 'itemable.rel_type="proposal" 
                              LEFT JOIN ' . db_prefix() . 'estimates ON ' . db_prefix() . 'estimates.id = ' . db_prefix() . 'itemable.rel_id AND ' . db_prefix() . 'itemable.rel_type="estimate"'
                            ];
            ...
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, [], [], 'GROUP by description');
