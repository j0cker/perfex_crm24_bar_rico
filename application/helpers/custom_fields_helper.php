...
function get_custom_field_value($rel_id, $field_id_or_slug, $field_to, $format = true)
{
    ...
    */
    ...
    //log_activity('get_custom_field_value PDF rel_id: ' . $rel_id .  ' field_id_or_slug: ' . $field_id_or_slug .' field_to: '. $field_to);
    $CI = & get_instance();

    $CI->db->select(db_prefix() . 'customfieldsvalues.value');
    ...
    if (is_numeric($field_id_or_slug)) {
        //log_activity('get_custom_field_value entra a slug');
    ...
    //log_activity('get_custom_field_value: ' . $result);
