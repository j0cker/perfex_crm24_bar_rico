...
{
    log_activity('_info_format_custom_field FINAL VALUE id: ' . $id .  ' label: ' . $label .' value: '. $value .' txt: '.$txt);
    

    if ($value != '') {
        //log_activity('_info_format_custom_field Detecta Valor ');
    ...
    //log_activity('_info_format_custom_field Resutaldo: ' . $result);
    ...
    function format_proposal_info($proposal, $for = '')
    {
        ...
        */
        ...
        foreach ($customFieldsProposals as $field) {
            ...
            //log_activity('_info_format_replace value PDF: ' . $value);
            $format = _info_format_custom_field($field['id'], $field['name'], $value, $format);
        }
